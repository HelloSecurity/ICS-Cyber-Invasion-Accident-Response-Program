...
<%
  ...
  if(infringement_level==0){
      PrintWriter script=response.getWriter();
      script.println("<script>");
      script.println("alert('infringement_level==0')");
      script.println("history.back()");
      script.println("</script>");
      script.close();
      return;
  ...
  //모든 입력사항을 다 입력한 경우 
               ...
               int result = scenarioDAO.ScenarioWrite(new ScenarioItem(0,scenario_name, infringement_level, asset_name, scenario_condition, scenario_sum, scenario_content));
               
               if(result==-1){
                  PrintWriter script=response.getWriter();
                  script.println("<script>");
                  script.println("alert('시나리오 추가에 실패했습니다.')");
                  script.println("</script>");
